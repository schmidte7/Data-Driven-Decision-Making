...
```{r}
# Compute the VIF value (explained)
VIF = 1/(1-r_squared)
VIF
```
...
To properly analyze the results in the table above, we need to ensure that the variables to not exceed the average of VIF. Multicollinearity is present when certain VIF are larger than 5 or 10. The VIF in this data definitely exceeds at 20.07, showing signs of  multicollinearity between the number of solar panels and manufacturing cost. Additionally, the tolerance value or unique variance is approximately 5.0%, while the explained would be roughly 95.0%. **Therefore, we can conclude we do have to do a model selection.**
...
model <- lm(manufacturing_cost ~ log(number_of_solar_panels), data = helio)

# Expected average manufacturing cost per solar panel for 400 solar panels
man_cost <- predict(model, newdata = data.frame(number_of_solar_panels = 400))
man_cost
...
mean(c(man_cost_4700, man_cost_4800, man_cost_4900, man_cost_5000))
```
