...
```{r, message=FALSE, warning=FALSE}
histogram <- ggplot(gather(data = helio),aes(x = value)) +
...
```{r}
# CODE NEEDS TO BE FIXED # Compute the VIF value (explained)
#model <- with(Log_cost ~ Log_number, data = helio)
#vif(model) # Call VIF from the source
```


```{r}
# Compute the tolerance value (unique)
#tolerance_value = 1 - VIF
#tolerance_value
```

To properly analyze the results in the table above, we need to ensure that the variables to not exceed the average of VIF. Multicollinearity is present when certain VIF are larger than 5 or 10. None of the variables show signs of multicollinearity. **Therefore, we can conclude we do not have to do a model selection.**
...
```
**WILL ADJUST COMMENTS ONCE I FIGURE OUT VIF**
**Residuals vs Fitted**\
...
**Normal Q-Q**\
To check the normality of the errors \(ε_i\), analyze a normal Q-Q plot of the residuals. There are three values that slightly depart by the tail. Additionally, a zigzag pattern hugs the regression line and it is difficult to say whether or not we would consider this data normally distributed.**INCOMPLETE**

**Scale-Location**\
As seen in the other graphs, there is an extreme residual. We know that this is considered extreme because  of the following: 2xσ.**INCOMPLETE** 

**Residuals vs Leverage**\
The (40) outlier has a big influence as it is underneath the 1 line to the right that refers to Cook’s distance. **INCOMPLETE**\
