...
**Variance Inflarion Factors**
```{r}
# CODE NEEDS TO BE FIXED
#VIF(helio[,2], helio[,1]) # Call VIF from the source
```
...
```{r}
par(mfrow = c(1,4)) # Organizes graphs in a specific order
PlotResidX(lin_reg_helio[, 1:2], lin_reg_helio$residuals, col= "blue") # Creates the residuals plots and utilized color
```

```{r}
# Function to graph autocorrelations
acf(resid(lin_reg_helio), main="Residuals Autocorrelation: LS") 
```
